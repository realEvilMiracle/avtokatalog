...
$query = mysqli_query($connect, 'SELECT * FROM spares');
...
if(isset($_POST["add_to_cart"]))
{
	...
	else
	{
		...
		if($_POST["quantity"] > $row['kolichestvo'])
		{
			unset($_SESSION["shopping_cart"][$count]);
			echo '<script>alert("Такого количество товара на складе нет!")</script>';
	...
	<header class="header">
            ...
            <div class="header_body">
                ...
                <nav class="header_menu">
                    <ul class="header_list">
						...
						<li><a href="productionSearch1.php" class="header_link">Поиск товара</a></li>
						<li><a href="#корзина" class="header_link">Корзина</a></li>
                    </ul>
                </nav>
            </div>
        </div>
		...
		<!-- <div class="col-md">
			...
			<div class="col-md-4">
				<form method="post" action="zxc.php?action=add&id=<?php echo $row["id_zapchasti"]; ?>">
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align="center">
						...
						<h4 class="text-danger">Кол-во:<?php echo $row["kolichestvo"]; ?></h4>
			...
			<div class="table-responsive">
				...
				<form method="post">
				<table class="table table-bordered">
					<td><input type="text" name="Comments" placeholder="Напишите каомментарий"></td>
					<td><input type="text" name="order_id" placeholder="Введите ваш уникальный id"></td>
...
if ( !$order_id || !$Comments ) {
    echo 'Вы ввели не все данные.';
    exit();
}
$query = "INSERT INTO order_spares (Comments,order_id, id_zapchasti)
    values ('$Comments', '$order_id','')";
$result = mysqli_query($connect, $query);
if ($result) {
    echo "Нов";
}
